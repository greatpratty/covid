...
Here we build a new column with the total of confirmed and probable cases, deaths and hospitalizations.
...
Build 3 new dataframes for the different regions in Virginia:
...
Total Population in the Eastern Planning Region: 1,854,806. 
...
```
```{r}
percent_hadit_Eastern = ggplot(data = casesEastern, aes(x = FixedDate, y = numberHadit)) + geom_line()
